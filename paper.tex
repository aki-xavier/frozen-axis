...
\hline
Lemma 1, general form, \eqref{eq:focgeneral} onward & \texttt{general\ttu proportionality}, \texttt{scalar\ttu family\ttu iff\ttu ratio\ttu agrees}, \texttt{frozen\ttu family\ttu two\ttu configs\ttu iff}, \texttt{position\ttu wise\ttu escape\ttu witness}, \texttt{KappaPhi\ttu quadratic}, \texttt{general\ttu first\ttu order\ttu of\ttu local\ttu min} & A\\
\hline
...
The derivation of the general first-order condition from the objective, which is what makes the general form's premise a consequence rather than an assumption. The statements are given in full because this is the one place where analysis, rather than algebra, enters the verification:
\begin{quote}
\small
\begin{verbatim}
theorem deriv_odd_of_even {phi phi' : R -> R}
    (heven : forall x, phi (-x) = phi x)
    (hphi : forall x, HasDerivAt phi (phi' x) x) (x : R) :
    phi' (-x) = -phi' x

theorem PairwiseTerm_hasDerivAt {phi phi' : R -> R}
    (heven : forall x, phi (-x) = phi x)
    (hphi : forall x, HasDerivAt phi (phi' x) x)
    (d : Depth) (i : Pixel) :
    HasDerivAt (fun t => PairwiseTerm phi (Function.update d i t))
      (KappaPhi phi' d i) (d i)

theorem dataTerm_hasDerivAt {f f' : Pixel -> R -> R}
    (hf : forall k x, HasDerivAt (f k) (f' k x) x)
    (d : Depth) (i : Pixel) :
    HasDerivAt (fun t => sum k, f k (Function.update d i t k))
      (f' i (d i)) (d i)

theorem GeneralObjective_hasDerivAt {phi phi' : R -> R}
    {f f' : Pixel -> R -> R}
    (heven : forall x, phi (-x) = phi x)
    (hphi : forall x, HasDerivAt phi (phi' x) x)
    (hf : forall k x, HasDerivAt (f k) (f' k x) x)
    (lambda : R) (d : Depth) (i : Pixel) :
    HasDerivAt (fun t => GeneralObjective phi f lambda
      (Function.update d i t))
      (f' i (d i) + lambda * KappaPhi phi' d i) (d i)

theorem general_first_order_of_local_min {phi phi' : R -> R}
    {f f' : Pixel -> R -> R}
    (heven : forall x, phi (-x) = phi x)
    (hphi : forall x, HasDerivAt phi (phi' x) x)
    (hf : forall k x, HasDerivAt (f k) (f' k x) x)
    (lambda : R) (d : Depth) (i : Pixel)
    (hmin : IsLocalMin (fun t => GeneralObjective phi f lambda
      (Function.update d i t)) (d i)) :
    f' i (d i) + lambda * KappaPhi phi' d i = 0

theorem KappaPhi_quadratic (d : Depth) (i : Pixel) :
    KappaPhi (fun x => 2 * x) d i = 4 * kappa d i

theorem FirstOrder_of_quadratic_local_min (x d : Depth)
    (lambda : R) (i : Pixel)
    (hmin : IsLocalMin (fun t => GeneralObjective (fun y => y ^ 2)
      (fun k t => (x k - t) ^ 2) lambda
      (Function.update d i t)) (d i)) :
    FirstOrder x d (2 * lambda) i
\end{verbatim}
\end{quote}

Note what the last statement says. The paper's stationarity condition of Lemma~1 assumes the first-order form and normalizes the constant factors by hand; here the condition is obtained from the quadratic objective as it stands, and the normalization is an explicit scalar: stationarity at a local minimum along a coordinate line holds exactly with the scalar \texttt{2 * lambda}, because the data term contributes \texttt{2 (d\ttu i - x\ttu i)} and the squared potential contributes \texttt{4 * lambda * kappa\ttu i}. The premise is thereby derived rather than assumed, and the derivation uses no property of the carrier beyond the differentiability of the potential and of the data term.
